...
private HashMap<String,String> convertActionsToHashMap(String[] singleAction){
	...
	public void initializeWebTable(String tableAttribute, int tableIndex, int documentIndex, int formIndex,String listPath, int syncTime) throws Exception {
		...
		}else if(tableAttribute.contains("class=")){
			tablePath = docPath+"/web:table[@class='"+tableAttribute.split("=")[1]+"']";
		}
		
		DOMDocument d = web.document(docPath);
		PropertyTestList prop = new PropertyTestList();
		prop.add("class", "x1o");
		List<DOMElement> ele = d.getElementsByTagName("table", prop);
		System.out.println("ele :"+ele.size());
		System.out.println("tablePath :"+tablePath);
		
		System.out.println(web.table(tablePath).exists());
