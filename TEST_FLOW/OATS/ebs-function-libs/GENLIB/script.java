...
public class script extends IteratingVUserScript {
	...
	public void run() throws Exception {
		...
		String abc = "Once the document is Cancelled, it cannot be undone. Do you want to proceed";
		String def = "Once the document is Cancelled.*it cannot be undone.*Do you want to proceed.*";
			
		boolean check = compareStrings(def,abc);	 
	...
	public boolean formVerifyChoiceBox(String expectedValue) throws Exception {
        ...
        if (forms.choiceBox(xPath).exists(15, TimeUnit.SECONDS)) {
        	...
        	//info("Actual Text after new line removal 	:" + Text);	
...
//		{
//			needPrompt	=	false;
//			getVariables().set("oracle_jtt_url",getSettings().get("oracle_jtt_url"));
//		}
//			
//		if(getSettings().get("oracle_telnet_url")!=null)
//		{
//			needPrompt	=	false;
//			getVariables().set("oracle_telnet_url",getSettings().get("oracle_telnet_url"));
//		}
//				
//		if(needPrompt)
//		{
...
//			
//			url = javax.swing.JOptionPane.showInputDialog("Provide Telent Server With Host");
//			getVariables().set("oracle_telnet_url",url);
//		}
	}
...
//	  		 System.out.println("Year out"+cal.getTime());
	...
	public void setProperty(String key, String value) throws Exception{
        ...
        getVariables().set(key, value,Variables.Scope.GLOBAL);
        ...
        if(f.exists()){
        	...
        	// Variable File Path
        	 ...
        	 //info("variablesFilePath :\""+variablesFilePath+"\"  flowName :\""+flowName+"\"");
	...
	public void formsSendKey(String xpath, String value) throws Exception {
		forms.textField(xpath).click();
		forms.textField(xpath).setText(value);
	}
	...
	public boolean formVerifyEdit(String editLogicalNameXPath, String expectedValue) throws Exception {
		...
		if (forms.textField(editLogicalNameXPath).exists(5, TimeUnit.SECONDS)) {
			...
			{ 
				...
				// Compate strings by trimming Text
				boolean verify2	=	 compareStrings(expectedValue, Text.trim());
				
				// Compate strings by trimming expectedValue
				boolean verify3 = false;
				if(expectedValue!=null && !"".equals(expectedValue))
					verify3	=	compareStrings(expectedValue.trim(), Text.trim());
				
				/**
				 * Developed for the issue: The text :::>264955*2 doesn't match with the expected value :::>264955*2 
				 * Here both texts are same but failing. Hence written below code
				 */
				boolean verify4 = false;
				if(expectedValue!=null && !"".equals(expectedValue)){
					if(expectedValue.equals(Text)){
						info("true");
						verify4	= true;
					}
				}
...
//       browser.launch();
	...
	public void clickLink(String linkName, String windowName, int compSequence, boolean isFormPathExists)
		...
		links = getLinkNames(linkName,	linkAttr,isFormPathExists);
	...
	public void webClickButton(String buttonName) throws Exception {
		...
		/* Document Path */
		String webIndex = web.getFocusedWindow().getAttribute("index");
		String title = web.getFocusedWindow().getTitle();

		String windowPath = "/web:window[@index='" + webIndex + "' or @title='" + title + "']";
		//String windowPath = getCurrentWindowXPath();

		int currentWindowIndex = Integer.parseInt(webIndex);
		int documentIndex = 0;
		int formIndex = -1;
		int buttonIndex = 0;
		String formName = "";
		...
		for (int i = 0; i < buttonList.size(); i++) {
			...
			if(value == null){
				value = button.getAttribute("text");
			}
	...
	/**
	 ...
	 public void webSetTextWithLOV(String textFieldAttributes, String searchByOption, String searchValue, String colName, String rowValue) throws Exception
	 {
			...
			boolean searchWindowExists = web.window("/web:window[@title='Search and Select List of Values']").exists(15, TimeUnit.SECONDS);
	...
	public void webSelectLOV(String lovName, String searchByOption, String searchValue, String colName, String rowValue)
		...
		if (colNames.length == rowValues.length) {
			...
			webSelectFromLOV(lovName,searchByOption,searchValue,colNames,rowValues);
		} else {
			info("Unable to select the value from WebSelectLOV table as the values provided not mathcing:");
			this.sop("Column Names" + Arrays.asList(rowValues).toString() + "    RowValues :" + Arrays.asList(rowValues).toString());
		}
	}
	...
	public void webSelectFromLOV(String lovName, String searchByOption, String searchValue, String[] colNames,
		...
		System.out.println("******************** Start of webSelectLoV ************************");
		info("******************** Start of webSelectLoV ************************");
		...
		info("Current Window Title :"+currentWindowTitle);
		...
		/*if ((lovName != null) && !("".equals(lovName))) {// When LOV Name is available
			...
			try{
				web.window("/web:window[@title='"+currentWindowTitle+"']").waitForPage(400,false);
				
				web.window("/web:window[@title='"+currentWindowTitle+"']").exists(100, TimeUnit.SECONDS);
				
			}catch(Exception e){
				web.window("/web:window[@title='"+currentWindowTitle+"']").waitForPage(400,true);
			...
			try{
				web.window("/web:window[@index='"+currentWindowIndex+"']").waitForPage(400,false);				
				web.window("/web:window[@index='"+currentWindowIndex+"']").exists(100, TimeUnit.SECONDS);
				if(currentWindowIndex>0)
				{
					web.window("/web:window[@index='"+(currentWindowIndex-1)+"']").waitForPage(400,false);				
					web.window("/web:window[@index='"+(currentWindowIndex-1)+"']").exists(100, TimeUnit.SECONDS);
				}
				
			}catch(Exception e){
				web.window("/web:window[@index='"+currentWindowIndex+"']").waitForPage(400,true);				
				web.window("/web:window[@index='"+currentWindowIndex+"']").exists(100, TimeUnit.SECONDS);
				if(currentWindowIndex>0)
				{
					web.window("/web:window[@index='"+(currentWindowIndex-1)+"']").waitForPage(400,true);				
					web.window("/web:window[@index='"+(currentWindowIndex-1)+"']").exists(100, TimeUnit.SECONDS);
				}				
			}
...
*/		
	...
	public String formTableSearch(HashMap<String, String> searchCol, int visiblelines) throws Exception {
		...
		//System.out.println("Column Paths :"+Arrays.asList(attrVal).toString());
		info("Expected Column Values :"+Arrays.asList(colVals).toString());
		...
		for (int iCounter = 0; !isBlankRow(iCount,attrVal) ; iCounter++) {
			...
			if (firstCell.trim().equalsIgnoreCase(colVals[0].trim())) {
				...
				for (int j = 1; j < colVals.length; j++) {
					...
					//this.sop("Expected value :"+colVals[j]);
					
					if (!((colVals[j]).equalsIgnoreCase(otherCell))) {
						bFound = false;
						break;
					}else{
						//info("Actual text \""+otherCell+" is not matched with expected text \""+colVals[j]);
					}
				}
	...
	public boolean formVerifyRadioButton(String path, String radioButtonStatus) throws Exception {
		...
		if (forms.radioButton(xPath).exists()) {
			actualStatus = forms.radioButton(xPath).isSelected();

			if (expectedStatus == actualStatus) {
				info("Radio button selection status :\""+actualStatus+"\" is matched with expected radio button status:\""+expectedStatus+"\"");
				actualStatus = true;

			} else {
				reportFailure("Radio button selection status :\""+actualStatus+"\" is not matched with expected radio button status:\""+expectedStatus+"\"");
				actualStatus = false;
...
*/
	...
	@SuppressWarnings("unchecked") public void setValueBasedonLabelAfterUIComponent(String labelName, String compType,	String valueToSet) throws Exception {
	...
	public void formSetValueInDynamicColumn(String columnName, String rowNumStr, String valueToSet) throws Exception {
		...
		// Activate table
		forms.window("//forms:window[(@name='Q_RES')]").activate(true);

		// Clicking on the cell
		forms.textField("//forms:textField[(@name='" + xPath + "')]").click();

		// Setting text in the specified cell
		forms.textField("//forms:textField[(@name='" + xPath + "')]").setText(valueToSet);
        //**************************** START OF BUG 1035      *********************
	...
	public boolean verifyValueInDynamicColumn(String columnName, String rowNumStr, String expectedValue)
		...
		String cellValue = getValueInDynamicColumn(columnName,	rowNumStr);

		if (cellValue.equals(expectedValue)) {
			textVerified = true;
			info("actual cell value \""+cellValue+"\" is matched with expected vlaue \""+expectedValue+"\"");
		}
	...
	public String getValueInDynamicColumn(String columnName, String rowNumStr) throws Exception {
		...
		// Getting Column Names & Count
		for (int index = 1;; index++) {// previously index = 2 change to '0' as in "View Quality Results" index starts from 1
																...
																// web
	{
		...
		if (web.confirmDialog("/web:dialog_confirm[@text='*' and @index='0']").exists(20,TimeUnit.SECONDS)) {
			
			message = web.confirmDialog("/web:dialog_confirm[@text='*' and @index='0']").getAttribute("text");
			
			info("Confirmation Message :"+message);
...
*/
	...
	public void selectListMultiValues(String labelName, String options) throws Exception {
		...
		options=Pattern.compile("\\,",Pattern.LITERAL).matcher(options).replaceAll(Matcher.quoteReplacement("~~"));
        String arr[] = options.split(",");
