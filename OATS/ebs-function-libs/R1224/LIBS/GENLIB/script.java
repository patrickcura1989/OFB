	...
	public void webSelectFromLOV(String lovName, String searchByOption, String searchValue, String[] colNames,
		...
		if ((lovName != null) && !("".equals(lovName))) {// When LOV Name is available
			...
			try{
				...
				web.window("/web:window[@title='"+currentWindowTitle+"']").waitForPage(200,false);				
				web.window("/web:window[@title='"+currentWindowTitle+"']").exists(100, TimeUnit.SECONDS);
			}catch(Exception e){
				web.window("/web:window[@title='"+currentWindowTitle+"']").waitForPage(200,true);				
				web.window("/web:window[@title='"+currentWindowTitle+"']").exists(100, TimeUnit.SECONDS);
			}
			...
			try{
				web.window("/web:window[@index='"+currentWindowIndex+"']").waitForPage(200,false);				
				...
				web.window("/web:window[@index='"+currentWindowIndex+"']").waitForPage(200,true);				
				web.window("/web:window[@index='"+currentWindowIndex+"']").exists(100, TimeUnit.SECONDS);
				/*if(currentWindowIndex>0)
				{
...
*/
	...
	public boolean verifyRequestStatus(String expectedValue) throws Exception {
		String requestPhase = forms.textField("//forms:textField[(@name='JOBS_PHASE_0')]").getText();
		String requestStatus = forms.textField("//forms:textField[(@name='JOBS_STATUS_0')]").getText();
		boolean retVal = false;
		int i = 1;
		while ((!requestPhase.equals("Completed")) && (i < 500)) 
		{
			forms.button("//forms:button[(@name='JOBS_REFRESH_0')]").click();
			requestPhase = forms.textField("//forms:textField[(@name='JOBS_PHASE_0')]").getText();
			requestStatus = forms.textField("//forms:textField[(@name='JOBS_STATUS_0')]").getText();
			i++;
			delay(2000);
		}
