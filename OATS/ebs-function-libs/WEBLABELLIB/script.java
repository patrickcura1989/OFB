...
public class script extends IteratingVUserScript {
	...
	public void webSetTextBasedOnLabel(String labelWithIndex, String valueToEnter)throws Exception {
		...
		if(textFieldElement != null){
			...
			warn("Unable to find the Text Field "+textFieldElement+"  element");
		}
	}
	...
	public String webGetEditTextBasedOnLabel(String labelWithIndex)throws Exception 
	{
		...
		for(int index=0; index<components.length; index++){
			...
			if(textElement != null){
				...
				System.out.println("type:"+components[index]+"    value:"+value);
				
				if(value != null){
					value = value.trim();
					break;
				}else{// value null means there is no value in the edit field
					value = "";
				}
			}else if(index == components.length-1 && textElement == null){
				
				reportFailure("Unable to find the text field for the specified label :"+labelWithIndex);
	...
	{
		// Label Components
		String[] components = {"div","span","h2","label","td","input"};//if td is there at the beginning of the array. Script is throwing null pointer exception. hence move to last
		...
		for(int compIndex=0; compIndex < components.length; compIndex++){
			...
			//List<DOMElement> compList = web.document(docPath).getElementsByTagName(components[compIndex], propertyTestList);
			//System.out.println("comp type: "+components[compIndex]);
