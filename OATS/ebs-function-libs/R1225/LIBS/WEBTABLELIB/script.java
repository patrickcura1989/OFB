...
public class script extends IteratingVUserScript {
	...
	public String tableactions(String tableName, HashMap<String, String> searchColumns, List <String[]> actions)throws Exception {
		...
		if(tableName.contains("id=") || tableName.contains("name=") || tableName.contains("summary=")){
		...
		for(int i=0; i<actions.size();i++){
			...
			if(action.get("keyword").equals("CHECK")){  // Verification of keyword
				...
				if(action.get("object").equals("GENLIB")){
					...
					}else if(action.get("funcname").equalsIgnoreCase("HANDLEDIALOG")){
						
						System.out.println("*********  Start of Handle Dialog in Table ****************** ");
						
						Thread.sleep(10000);
						
						gENLIB.handleDialog(action.get("value1"));
						
						System.out.println("*********  End of Handle Dialog in Table  ****************** ");
					}
					else
					{
