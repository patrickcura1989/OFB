...
*/	
	...
	public String getLineNumberOfTheItem(String itemID,String searchValues) throws Exception {
		
		 int documentIndex = 0;
	     String eleAttr="table";
	     String attrVal="";
	        
        String windowPath ="/web:window[@index='"+web.getFocusedWindow().getAttribute("index")+"']";
        int currentWindowIndex = Integer.parseInt(web.window(windowPath).getAttribute("index"));
       
        String tableDetails[] = itemID.split(",");      
        for (int index = 0; index < tableDetails.length; index++) {
              ...
              if (tableDetails[index].trim().startsWith("document=")) 
              {
                    ...
                    String[] table  = tableDetails[index].split("=");

                    if(table.length==2){
                          eleAttr=table[0].replace("@", "");
                          attrVal=table[1];
                    }else{
                          attrVal=table[0];
                    }
        ...
        String tablePath=windowPath + "/web:document[@index='" + documentIndex + "']";
        System.out.println("Table Path :"+tablePath);
        
        List<DOMElement> sourceList=new ArrayList<DOMElement>();
        String types[]={"a","span","div"};
        
        for(int i=0;i<types.length;i++){
        	List<DOMElement> eList= web.document(tablePath).getElementsByTagName(types[i]);
        	sourceList.addAll(eList);
        }
        
        List<DOMElement> finalList = new ArrayList<DOMElement>();
        
        /*Store links which match search Results*/
        //int index = 0;
        if(sourceList.size()!=0){
              
              for(DOMElement t: sourceList){
                    String value= t.getAttribute("text");
                    if(value!=null){
                          
                          if(value.equals(searchValues)){
                                finalList.add(t);
                          }
                    }
              ...
              for(DOMElement t: finalList){
                    String value= t.getAttribute("id");
                    
                    if(value!=null){
                          
                          String index = value.replace(attrVal.replaceFirst(":0", ":"), "").trim();
                          int line=Integer.parseInt(index);
                          getVariables().set("setLine", index+"");
                          return ""+line;
                    }
              ...
              info("Item :"+searchValues+"not found");
              getVariables().set("setLine", "-1");
              return ""+0;
        }
        getVariables().set("setLine", "-1");
        return ""+0;
	...
	/**
	 * This functions is used to find the row number based on Supplier & Supplier site and performs the following
	 * i) Check the checbox
	 * ii) Enter the values in firstname & last name based on the values provided
	 * iii) Click on Add Note 
	 * @param supplierName
	 * @param supplierSite
	 * @param fieldName
	 * @param valueToEnter
	...
	public void enterSupplierDetails(String supplierName, String supplierSite, String fieldName, String valueToEnter) throws Exception{
		...
		//Finiding Row Number and Selecting Checkbox
	...
	public void verifySupplierInformation(String supplierName, String supplierSite, String fieldName, String valueToVerify ) throws Exception{
        ...
        if(valToVerify!=null && valueToVerify.trim().equals(valToVerify.trim())){
              ...
              this.reportPassed("Verified the details: \""+supplierName + "\" with supplier site \""+ supplierSite + "\" has \"" + fieldName+ "\" value as \"" + valueToVerify +"\"");
              
        }else{
              
              this.reportFailure("Unable to veirfy the details : \""+supplierName + "\" with supplier site \""+ supplierSite + "\" doesn't  have \"" + fieldName+ "\" value as \"" + valToVerify + "\" , where as expected values is \"" + valueToVerify );
        }
	...
	public void verifySupplierDetails(String supplierName, String supplierSite, String fieldName, String valueToVerify ) throws Exception{
		...
		if(valToVerify.trim().equals(valueToVerify.trim())){
	...
	public void awardTableAction(String supplier, String awardOption, String objectType, String value)  throws Exception {
		...
		/*info("Idexes: checkbox : 0, radio:1, "+objIndex);
		info("Object Index :"+objIndex);*/
		if(objIndex < 0){
		...
		case 0: {
			...
			//awardTable.selectCheckBox("Label", awardOptions[i], supplier,toBoolean(actionVals[i]) );
				HashMap <String,String>search21619 = new HashMap<String,String>();
				List <String[]>action21619 = new ArrayList<String[]>();
				
				search21619.put("Label",awardOptions[i]);
				action21619.add( new String[]{"CHECK","CHECKBOX",supplier,supplier,"","",toBoolean(actionVals[i])+""});
				
				//String setLine=wEBTABLELIB.tableactions(supplier, action21619,search21619);
				String setLine=wEBTABLELIB.tableactions("Label", action21619,search21619);
		...
		case 1: {
				for(int i =0 ; i< awardOptions.length ; i++){
	                ...
	                search21619.put("Label",awardOptions[i]);
	                action21619.add( new String[]{"SELECT","RADIOBUTTON",supplier,supplier,"","",toBoolean(actionVals[i])+""});
...
//				awardTable.setText("Label", awardOptions[i], supplier,actionVals[i] );
                ...
                action21619.add( new String[]{"SETTEXT","EDIT",supplier,supplier,"","",actionVals[i]});
                //String setLine=wEBTABLELIB.tableactions(supplier,action21619,search21619);
                String setLine=wEBTABLELIB.tableactions("Label", action21619,search21619);
...
//				awardTable.setTextArea("Label", awardOptions[i], supplier,actionVals[i] );
                ...
                search21619.put("Label",awardOptions[i]);
                action21619.add( new String[]{"SETTEXT","TEXTAREA",supplier,supplier,"","",actionVals[i]});
	...
	public void clearShopppingCart() throws Exception{
		...
		System.out.println("*******  Start of clearShopppingCart ****** ");
		...
		if(web.exists("//web:button[@id='PopupCheckout']",100)){
		
			web.button("//web:button[@id='PopupCheckout']").click();
			
			think(5);
			
			// Waiting for the shopping Cart popup to open
			String docIndex = gENLIB.getDocumentIndex("Need-By Date","Need-By Date,type=label");
			
			String popDocPath =  getCurrentWindowXPath()+"/web:document[@index='{{docIndex}}']";
			String imagePath = popDocPath+"/web:image[@alt='Click this image to delete the line.']";
			
			while(web.exists(imagePath)){
				
				web.image(imagePath).click();
				
				Thread.sleep(3000);
				
				web.image(popDocPath+"/web:image[@alt='Return']").waitFor(60);
			}
		 
			web.image(popDocPath+"/web:image[@alt='Return']").click();
			
			web.window("/web:window[@title= 'Oracle iProcurement:*']").waitForPage(160, true);
		}
		...
		System.out.println("*******  End of clearShopppingCart ****** ");
	}
	...
	public void Award_Bid_To_Supplier(String supplierName , String award, String awardOption,String ValueToEnter, String InternalNote , String NotetoSupplier)throws Exception{
        ...
        if((award!=null)&&(!award.equals(""))){
        	...
        	search21619.put("Label","Award");
        	action21619.add( new String[]{"SELECT","RADIOBUTTON",supplierName,supplierName,"","","TRUE"});
        ...
        if((awardOption!=null)&&(!awardOption.equals(""))){
            ...
            search21619.put("Label",awardOption);
            action21619.add( new String[]{"SETTEXT","EDIT",supplierName,supplierName,"","",ValueToEnter});
        ...
        if((InternalNote!=null)&&(!InternalNote.equals(""))){
            ...
            search21619.put("Label","Internal Note");
            action21619.add( new String[]{"SETTEXT","TEXTAREA",supplierName,supplierName,"","",InternalNote});
        ...
        if((NotetoSupplier!=null)&&(!NotetoSupplier.equals(""))){
            ...
            search21619.put("Label","Note to Supplier");
            action21619.add( new String[]{"SETTEXT","TEXTAREA",supplierName,supplierName,"","",NotetoSupplier});
	...
	public String encryptURL(String ou) throws Exception{
		...
		searchColumns.put("Operating Unit", ou);
		...
		HashMap<String,String> getTextAction = new HashMap<String,String>();
		getTextAction.put("keyword", "GET");
		getTextAction.put("object","TEXT");
		getTextAction.put("displayname", "Encryption String"); 
		getTextAction.put("logicalname", "Encryption String");
		getTextAction.put("outputvar", "");
		getTextAction.put("funcname", "");
		
		
		// Getting WebTable Object
		String tableName = "Encryption String";
		
		wEBTABLEOBJ.getWebTableObject(tableName);
		
		int rowNumber = wEBTABLEOBJ.findRowNumber(searchColumns, tableName);
		
		List<String> encryptURL = wEBTABLEOBJ.getCellData(tableName, rowNumber, getTextAction);
		...
		if(encryptURL.size() <= 0){
			...
			wEBTABLEOBJ.checkOn(tableName, rowNumber, action);
		...
		encryptURL = wEBTABLEOBJ.getCellData(tableName, searchColumns, getTextAction);
		info("Encrypted Url: "+encryptURL);
		
		String appUrl = web.document("/web:window/web:document").getRecordedURL();
		
		String path = "//span[@id='UrlExample']";
		
		urlPattern = web.element(path).getAttribute("text");
...
public void selectApproverInManageApprovals(String approver) throws Exception{
	...
	for(int index=0; ; index++){
		...
		docPath = "/web:window[@index='"+windowIndex+"']/web:document[@index='"+index+"']";
		...
		if(!docExists){
			...
			prop.add("innertext", "* Actions");
			List<DOMElement> elements = web.document(docPath).getElementsByTagName("span", prop);
	...
	/*// Get the document index from popup	
	String sDocIndex = getVariables().get("DOCINDEX");
	
	//If DOCINDEX is null , then consider "0" as docindex
	if(sDocIndex != null){
		docIndex = Integer.parseInt(sDocIndex);
	}*/
	
	//String docpath ="/web:window[@index='"+index+"']/web:document[@index='"+docIndex+"']";
