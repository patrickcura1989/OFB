...
public class script extends IteratingVUserScript {
	...
	public HashMap<String,String> getColumnDetails(HashMap<String,String> actions) throws Exception{
		...
		String logicalName = actions.get("logicalname");
		
		colDetails = getColumnDetails(displayName);
		
		if(logicalName != null){
			colDetails = getColumnAttributeDetails(logicalName,colDetails);
		}
	...
	public HashMap<String,String> getColumnDetails(String columnName) throws Exception{
		...
		for (int index = 1; index < details.length; index++) {
			...
			if (details[index].trim().toLowerCase().startsWith("colindex=")) {
	...
	public HashMap<String,String> getColumnAttributeDetails(String tableColLogicalName, HashMap<String,String> colDetails) throws Exception{
		
		String postAction = "";
		
		String logicalName = tableColLogicalName;
		
		logicalName = Pattern.compile("\\,",Pattern.LITERAL).matcher(logicalName).replaceAll(Matcher.quoteReplacement("~~"));
		String details[] = logicalName.split(",");
		...
		for (int index = 1; index < details.length; index++) {
			...
			if (details[index].trim().toLowerCase().startsWith("postaction=")) {

				postAction = details[index].split("=")[1];
				
			}
		}
		
		colDetails.put("postAction", postAction);
	...
	public void checkOn(String tableName, int rowNum, HashMap<String,String> actions)throws Exception {
		...
		//info("colDetails :"+colDetails.toString());
...
////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void selectList(String tableName, int rowNum, HashMap<String,String>actions)throws Exception {
		...
		webTable.selectList(rowNum, colDetails.get("colName"), actions.get("value1"), colDetails.get("postAction"),toInt(colDetails.get("compSeq"))) ;
...
class WebTable {
	...
	public void initialize() throws Exception {
		...
		this.itv.info("doc path :"+docPath);
		...
		if("".equalsIgnoreCase(listPath)){
			...
			System.out.println("list :"+Arrays.asList(select).toString());
	...
	public String[] getColumns() throws Exception
	{
		...
		for(int i=0;i<actColNames.length;i++){
			...
			String colName = actColNames[i];
			//colName = colName.replaceAll("t(.*)", "").trim();//
			
			colNames[i] = colName.trim();
			colNamesWithoutTrim[i] = colName;
	...
	public int getColumnNumber(String colName) throws Exception {
		...
		//System.out.println("similarColIndex :"+similarColIndex);
		...
		if(!isANumber(actColName))
		{
			...
			for(int index=0, colMatchIndex=0; index<lColNames.size(); index++){
				...
				if(actColName.contains(".*")){
					
					if(compareStrings(actColName, lColNames.get(index))){
...
public String getImage(int rowNum, int colNum) throws Exception{
	...
	public void selectList(int rowNum, String colName,String itemToSelect, String postAction, int compSequence) throws Exception{
		
		/* Get the Matched Column Number*/
		int colNum = getColumnNumber(colName);
		
		/* Slect List with compSequence=0(first selectBox in the cell)*/
		selectList(rowNum, colNum, itemToSelect,postAction, compSequence);
	...
	public void selectList(int rowNum, int colNum,  String itemToSelect, String postAction, int compSequence) throws Exception{
		...
		try{
			...
			if(ret.equals("False")){
				...
				// Get window title after selecting list box
				// when list box value is selected, either 
				//1. Page may be same or 2.It may move to different page (Observed in procurement)
				if(postAction != null){
					
					if("".equalsIgnoreCase(postAction)){
						Thread.sleep(3000);
						String currentWindowTitle = this.getCurrentWindowTitle();
						
						itv.info("Window Name Before Selecting List Box :"+windowName+" Window Name after selecting list box :"+currentWindowTitle);
						
						if(this.windowName.equals(currentWindowTitle)){
							waitForColumn(this.tableColName);
						}
						
					}else if(postAction.equalsIgnoreCase("popup")){
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	public int getRowNumber (String[] colNames, String[] rowValues, boolean partialMatch) throws Exception{
		...
		if(tableListBoxExists()){
			...
			for(; listIndex < listCount; listIndex++){
				...
				//String[] selectedValue = web.selectBox(tableListPath).getSelectedValue();
			
				//System.out.println("Selected Value :"+Arrays.asList(selectedValue).toString());
	...
	public int getRowNum(String[] colNames, String[] rowValues, boolean partialMatch) throws Exception{
		...
		//System.out.println("lActualColNames : "+(lActualColNames.toString()));
		...
		for(int rowIndex=1; rowIndex < rowCount; rowIndex++){
			...
			for(int expColIndex = 0 ; expColIndex < colNames.length ; expColIndex++){
				...
				//System.out.println("Column Indext is : "+colIndex);
				/*int colIndex = -1;
		...
		for(int index=0;index<defComponents.length;index++){
			...
			for(int dataIndex = 0; dataIndex<data.length; dataIndex++){
				if(data[dataIndex] != ""){
					...
					//System.out.println("removedNewLineString1 :"+removedNewLineString);
					...
					//System.out.println("removedNewLineString2 :"+removedNewLineString);
					
					if(removedNewLineString.contains("Effort(") || removedNewLineString.contains("Cost(")){// This issue was occured in Projects Advacedpack flow where data is availalbe as "1.0 Effort(1.0 Effort)" in the flow "PJT_Workplan_Plan_To_Execution_FSVE"
						//alert("textContent inside if "+columnValue);
						//Not replacing any data
					}else{
						removedNewLineString = removedNewLineString.replaceAll("t\\(.*?\\)", "");//IE9 Change -- Used for Replacing t('5','6') or t(8,0) or t('5')
					}
					
					//This is old old Code. This is was replaced by if condition mentioned above
					//removedNewLineString = removedNewLineString.replaceAll("t\\(.*?\\)", "");//IE9 Change
					
					
					//System.out.println("removedNewLineString3 :"+removedNewLineString);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void selectTableList(int listIndex) throws Exception {
		...
		if("".equalsIgnoreCase(tableListPath) || tableListPath == null){
			...
			System.out.println("Available Table List Path :"+tableListPath);
			System.out.println("ListIndex :"+listIndex);
			...
			if(listIndex == 0){
				...
				System.out.println("Selecting List Box with index"+listIndex);
